...
resource "aws_ecs_task_definition" "app" {
  ...
  # dois usam o MESMO arquivo SQLite (catalog-db: Pedidos, Bibliotecas, ItensBiblioteca),
  # senão o Worker gravaria num banco separado e a API nunca veria as mudanças. Sem
  # host_path: cada task recebe um volume Docker "fresco", coerente com o SQLite já
  # sendo efêmero neste projeto (recriado a cada deploy).
  volume {
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        { name = "Messaging__Provider", value = "Sqs" }, # MassTransit usa Amazon SQS/SNS (ver MassTransitExtensions.cs / Worker/Program.cs)
    ...
    },
    {
      ...
      # projeto separado (Fiap.FCGames.Catalogo.Worker), imagem própria (tag
      # worker-latest no mesmo ECR). Sem porta exposta: não recebe tráfego HTTP,
      # não passa pelo API Gateway.
